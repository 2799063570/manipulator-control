...
/**
 * 测试: 采用 servoCartesian 跟踪一个轨迹,目标点下发时间间隔 10ms
 */
int exampleServoCartesian(RpcClientPtr cli)
{
    // 读取轨迹文件,cartesian-heart-L.txt 为S3的轨迹;
    auto filename = "../trajs/cartesian-heart-L.txt";
    ...
    auto cur_q = robot_interface->getRobotState()->getJointPositions();
    // 接口调用: 逆解
    auto ik_result =
        robot_interface->getRobotAlgorithm()->inverseKinematics(cur_q, traj[0]);
    if (std::get<1>(ik_result) != 0) {
        std::cout << "轨迹文件中的第一个路点逆解失败，返回值："
                  << std::get<1>(ik_result) << std::endl;
        return -1;
    }

    // 接口调用: 关节运动到轨迹中的第一个点，否则容易引起较大超调
    robot_interface->getMotionControl()->moveJoint(
        std::get<0>(ik_result), 80 * (M_PI / 180), 60 * (M_PI / 180), 0, 0);
    ...
    for (size_t i = 1; i < traj.size(); i++) {
        // 接口调用: 笛卡尔空间伺服运动
        robot_interface->getMotionControl()->servoCartesian(traj[i], 0.0, 0.0,
                                                            0.01, 0.0, 0.0);
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
