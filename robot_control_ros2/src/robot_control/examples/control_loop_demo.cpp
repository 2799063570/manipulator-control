...
int main()
{
    ...
    interpolator.setStateStart(start_state);
    interpolator.setStateGoal(goal_state);
    interpolator.calculate3Times();
    TrajectoryBuffer buffer(6);
    for (int i = 0, size = interpolator.getStateNum(); i<size; ++i)
    {
        buffer.pushRobotState(interpolator.getRobotState(i));
    }
